...
export async function initDb() {
  ...
  `);
    const studentCount = await pool.query('SELECT COUNT(*)::text AS count FROM students');
    ...
    const eventsCount = await pool.query('SELECT COUNT(*)::text AS count FROM events');
    if (Number(eventsCount.rows[0]?.count ?? 0) === 0) {
        for (const event of seedEvents) {
            await pool.query(`INSERT INTO events (event_name, place, date, time, description) VALUES ($1,$2,$3,$4,$5)`, event);
        }
    }
}
